...
%         disp(i);
    ...
    %         ref_patch = filt_im(i:x2,j:y2,1:3);
            ref_patch = filt_im(i:x2,j:y2,fno);
            % color
    %         P=zeros(8*8*3,250);
            % grayscale
            P=zeros(8*8,250);
...
%     psnr_vals(it) = psnrDenoised;
% end
% figure,plot(gauss_lvls,psnr_vals),xlabel('Gaussian noise levels'),ylabel('psnr values'),title('varying Gaussian noise levels');
