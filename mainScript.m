...
poiss = random('Poisson',20,size(f1));
noisy_im = f1 + gauss + poiss; 
b=rand(size(f1));
c=imnoise(b,'salt & pepper',0.3);
ind0=find(c==0);
ind1=find(c==1);
noisy_im(ind0) = 0;
noisy_im(ind1) = 255;
...
[m,n,p]=size(noisy_im);
filt_im=[];
v = VideoWriter('morenoise_color_noisy_carphone_qcif.avi','Uncompressed AVI');
open(v);
for iter=1:size(noisy_im,3)
    filt_im(:,:,iter) = medfilt2(noisy_im(:,:,iter));
end
for iter=1:K
    writeVideo(v,mat2gray(noisy_im(:,:,(iter-1)*3+1:(iter-1)*3+3)));
end
close(v);
...
psnr_vals = zeros(K,1);
% filt_im = filt_im2;
v = VideoWriter('morenoise_color_denoised_carphone_qcif.avi','Uncompressed AVI');
open(v);
for fno=1:K
    new_im=zeros(m,n,3);
    pca_im=zeros(m,n,3);
...
%     figure,imshow(mat2gray(filt_im(:,:,1)));
    ...
    %         ref_patch = filt_im(i:x2,j:y2,1:3);
            ref_patch = filt_im(i:x2,j:y2,(fno-1)*3+1:(fno-1)*3+3);
            % color
            P=zeros(8*8*3,5*K);
            % grayscale
%             P=zeros(8*8,5*K);
...
%                 patch=im2col(a,[8 8]);
            ...
            denoised_patch=reshape(denoised_vec,8,8,3);
            pca_im(i:x2,j:y2,1:3) = pca_im(i:x2,j:y2,1:3) + denoised_patch;
%             patch_count(i-3:i+3,j-3:j+3)=patch_count(i-3:i+3,j-3:j+3)+1;
    ...
    %         tou = rand(1)+1;
            ...
            % color
            newmu = (sqrt(8*8*3) + sqrt(5*K) )*sqrt(ratio)*sigmahat;
            % grayscale
...
%     filt_im2(:,:,fno)=mat2gray(new_im)*255;
    im = f1(:,:,(fno-1)*3+1:(fno-1)*3+3);
    cropped_im = im(1:m-8,1:n-8,1:3);
    final_im = mat2gray(new_im)*255;
    cropped_final = final_im(5:m-4,5:n-4,1:3);
    writeVideo(v,uint8(cropped_final));
    psnrDenoised = psnr(uint8(cropped_final),uint8(cropped_im));
    psnr_vals(fno)=psnrDenoised;
end
pca_denoised = mat2gray(pca_im)*255;
cropped_pca = pca_denoised(5:m-4,5:n-4,1:3);
...
im = f1(:,:,index:index+3);
cropped_im = im(1:m-8,1:n-8,1:3);
im = noisy_im(:,:,index:index+3);
cropped_noisy = im(1:m-8,1:n-8,1:3);
final_im = filt_im2(:,:,index:index+3);
cropped_final = final_im(5:m-4,5:n-4,1:3);
figure,imshow(mat2gray(cropped_im));
